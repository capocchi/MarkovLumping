...
def get_best_partition(k,partitionObject,S,P,Pi,splitting_choice,coordinate_choice):

    result = {k:[sys.maxsize, None, None]}
    
    #print(next(partitionObject.GetLabeled(k, 'NS', splitting_choice, coordinate_choice)))
    #print(next(partitionObject.GetLabeled(k, 'NS', splitting_choice, coordinate_choice)))

    ### compute all partitions for a given k (optional splitting_choice, coordinate_choice)
    pbar = tqdm(partitionObject.GetLabeled(k, 'NS', splitting_choice=splitting_choice, coordinate_choice=coordinate_choice), unit='part', position=k-1, total=None if splitting_choice else partitionObject.Count(k))
    for p in pbar:
        ...
        if kl<result[k][0]:
            result[k] = [kl, p, Q]
            pbar.set_description(f"Processing for k={k}, kl={kl :.4f}, par={''.join([c[-1][-1] for c in p])}")
...
if __name__ == '__main__':
    ...
    ### $ python main_parallel_2.py 4x4.dat 2 22 1212 
    ### get the KL of the partiton 1212 from 4x4 matrix
    ### $ python main_parallel_2.py 4x4.dat 2 22
    ### get the best partition with bi-classes (k=2) and a split (22)
    ### $ python main_parallel_2.py 4x4.dat 2
    ### get the best partition for  2 classes (k=2)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if c is not None:
                        cc = list(map(int, str(c)))
                        assert n==sum(cc), f"The sum of splitting choice {c}={sum(cc)} must be equal to the size of matrix the ({n})"
                        assert len(cc)==k, f'The number of splitting choice len({c})={len(cc)} must be equal to k ({k})'
                    ...
                    mapped_values = tqdm(pool.imap_unordered(partial(get_best_partition, partitionObject=partitionObject, S=S, P=P, Pi=Pi, splitting_choice=c,coordinate_choice=z), K), total=len(K))
                    ...
                    print(f"Best partition: {result[1]} ({''.join([c[-1][-1] for c in result[1]])})")
